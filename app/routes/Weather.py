...
import difflib
import json
from validation.Validation import UserInputDestinationValidation
...
# 	"""Fetch a JSON response from an external API URL."""
...
#Handle location
def _resolve_location(location, value_location):
	params = urlencode(
		{
			"name": location_query,
			"count": 10,
	...
	name_map = {result.get("name", ""): result for result in results}
	best_match = difflib.get_close_matches(location_query, list(name_map.keys()), n=1, cutoff=0.6)
	if best_match:
...
def create_destination():
	...
	value_location = data.get("value_location")
	...
	try:
		start, end = UserInputDestinationValidation._Usvalidate_date_range(start_date, end_date)
	except ValueError as exc:
		return jsonify({"error": str(exc)}), 400

	resolved_location = _resolve_location(location, value_location)
